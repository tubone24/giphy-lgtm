    ...
    <>
      <ImageList rowHeight={180}>
        <ImageListItem key="Subheader" cols={2} style={{height: 'auto'}}>
          <ListSubheader component="div">Search for <b>【{query}】</b></ListSubheader>
        </ImageListItem>
        {images.map((item) => (
          <ImageListItem key={item.originalUrl}>
            ...
            <ImageListItemBar
              ...
              actionIcon={
                <CopyToClipBoard text={createMarkdownLink(item.title, item.originalUrl)}>
                  <IconButton aria-label={`info about ${item.title}`} onClick={handleClickPaste} color="default">
                    <AssignmentIcon/>
                  </IconButton>
                </CopyToClipBoard>
              }
      ...
      <Button
        onClick={() => setOffset(offset < maxShowImages ? 0 : offset - maxShowImages)}
      ...
      <Button
        onClick={() => setOffset(offset > 4999 - maxShowImages ? 4999 : offset + maxShowImages)}
        style={{marginRight: "5px"}}
        color="primary"
        className={classes.button}
      ...
      <Snackbar open={openPasteSnackBar} autoHideDuration={2000} onClose={handleClosePaste}>
